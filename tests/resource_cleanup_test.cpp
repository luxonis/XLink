...
int main(int argc, char** argv) {
    ...
    for (int round = 0; round < cfg.rounds; ++round) {
        ...
        const auto stream = XLinkOpenStream(handler.linkId, kStreamName, static_cast<int>(payload.size() * 2));
        ...
        if (XLinkWriteData(stream, payload.data(), static_cast<int>(payload.size())) != X_LINK_SUCCESS) {
            server.join();
            XLinkRemoveLinkDownCb(callbackId);
            return -1;
        }
