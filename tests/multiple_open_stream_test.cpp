...
int runServer(const std::string& endpoint, int numStreams) {
    ...
    for (int i = 0; i < numStreams; ++i) {
        threads.emplace_back([&, i]() {
            ...
            const auto stream = XLinkOpenStream(handler.linkId, name.c_str(), 1024);
...
int runClient(const std::string& endpoint, int numStreams) {
    ...
    for (int index : order) {
        threads.emplace_back([&, index]() {
            ...
            streams[index] = XLinkOpenStream(handler.linkId, name.c_str(), 1024);
        });
    }
    for (auto& thread : threads) {
        thread.join();
    ...
    for (int index : order) {
        threads.emplace_back([&, index]() {
            ...
            const auto status = XLinkReadData(stream, &packet);
