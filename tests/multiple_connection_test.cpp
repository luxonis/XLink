...
int runClientConnection(const std::string& endpoint, int connectionId, int numStreams, int numPackets) {
    ...
    for (int index : order) {
        threads.emplace_back([&, index]() {
            ...
            streams[index] = XLinkOpenStream(handler.linkId, name.c_str(), sizeof(kDummyData) * 2);
        });
    }
    ...
    for (int index : order) {
        threads.emplace_back([&, index]() {
            ...
            for (int packet = 0; packet < numPackets; ++packet) {
                if (XLinkWriteData(stream, kDummyData, sizeof(kDummyData)) != X_LINK_SUCCESS) {
                    success.store(false);
                    return;
                }

                streamPacketDesc_t desc = {};
                const auto status = XLinkReadMoveData(stream, &desc);
            ...
            if (XLinkCloseStream(stream) != X_LINK_SUCCESS) {
                success.store(false);
            }
        });
    }
...
int runServerConnection(const std::string& endpoint, int numStreams, int numPackets) {
    ...
    for (int i = 0; i < numStreams; ++i) {
        ...
        streams[i] = XLinkOpenStream(handler.linkId, name.c_str(), sizeof(kDummyData) * 2);
        ...
        writerThreads.emplace_back([&, i]() {
            ...
            for (int packet = 0; packet < numPackets; ++packet) {
                const auto status = XLinkWriteData2(stream,
...
int main(int argc, char** argv) {
    ...
    testutils::ProcessWatchdog watchdog(timeoutMs, "multiple_connection_test");
