...
XLinkError_t XLinkWriteFD_(streamId_t streamId, long *fd, XLinkTimespec* outTSend) {
    ...
    XLINK_INIT_EVENT(event, streamIdOnly, XLINK_WRITE_FD_REQ,
        size,(void*)fd, link->deviceHandle);
...
XLinkError_t XLinkReadFD(streamId_t const streamId, long *fd)
{
    ...
    xLinkEvent_t event = {0};
    XLINK_INIT_EVENT(event, streamIdOnly, XLINK_READ_FD_REQ,
        0, NULL, link->deviceHandle);
